...
const fs = require("fs-extra"); //const tinyurl = require("tinyurl");
...
const onChat = async ({ api, event }) => {
  ...
  try {
    if (
      ...
      await fs.ensureDir(cacheDir); // Ensuring the cache directory exists

      const path = cacheDir + `/video${ex}`;
      fs.writeFileSync(
        path,
        Buffer.from((await axios.get(d.result, { responseType: "arraybuffer" })).data, "binary")
      );
